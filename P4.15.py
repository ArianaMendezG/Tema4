...
def modulador_8_PSK(bits, fc, mpp):
    ...
    Tc = 1 / fc  # periodo [s]
    t_periodo = np.linspace(0, Tc, mpp)  # mpp: muestras por período
    porta_seno = np.sin(2*np.pi*fc*t_periodo)
    porta_coseno = np.cos(2*np.pi*fc*t_periodo)
    ...
    for i in range(0, N, 3):
        # Se define que b1=bits[i], b2=bits[i+1] y b3=bits[i+2]
        vector_bits = [bits[i],bits[i+1],bits[i+2]]
        
        if vector_bits == [0,0,0]:
            A1 = -h
            A2 = -h
            senal_Tx[i*mpp: (i+1)*mpp] = A1*porta_coseno + A2*porta_seno
            
        if vector_bits == [0,0,1]:
            A1 = -1
            A2 = 0
            senal_Tx[i*mpp: (i+1)*mpp] = A1*porta_coseno + A2*porta_seno
            
        if vector_bits == [0,1,0]:
            A1 = 0
            A2 = 1
            senal_Tx[i*mpp: (i+1)*mpp] = A1*porta_coseno + A2*porta_seno

        if vector_bits == [0,1,1]:
            A1 = -h
            A2 = h
            senal_Tx[i*mpp: (i+1)*mpp] = A1*porta_coseno + A2*porta_seno

        if vector_bits == [1,0,0]:
            A1 = 0
            A2 = -1
            senal_Tx[i*mpp: (i+1)*mpp] = A1*porta_coseno + A2*porta_seno

        if vector_bits == [1,0,1]:
            A1 = h
            A2 = -h
            senal_Tx[i*mpp: (i+1)*mpp] = A1*porta_coseno + A2*porta_seno
        
        
        if vector_bits == [1,1,0]:
            A1 = h
            A2 = h
            senal_Tx[i*mpp: (i+1)*mpp] = A1*porta_coseno + A2*porta_seno

        if vector_bits == [1,1,1]:
            A1 = 1
            A2 = 0
            senal_Tx[i*mpp: (i+1)*mpp] = A1*porta_coseno + A2*porta_seno
    ...
    return senal_Tx, P_senal_Tx, porta_coseno, porta_seno
...
def demodulador_8_PSK(senal_Rx, porta_coseno, porta_seno, mpp):
    '''Un método que simula un bloque demodulador
    ...
    # Vector para la señal demodulada
    senalcos_demodulada = np.zeros(senal_Rx.shape)
    senalsen_demodulada = np.zeros(senal_Rx.shape)
    
    # Pseudo-energía de un período de la portadora coseno:
    Es_cos = np.sum(porta_coseno * porta_coseno)
    
    # Pseudo-energía de un período de la portadora seno:
    Es_sen = np.sum(porta_seno * porta_seno)
    ...
    for i in range(N):
        # Producto interno de dos funciones
        # Producto interno portadora coseno:
        producto_cos = senal_Rx[i*mpp: (i+1)*mpp] * porta_coseno
        Ep_cos = np.sum(producto_cos)

        # Producto interno portadora seno:
        producto_sen = senal_Rx[i*mpp: (i+1)*mpp] * porta_seno
        Ep_sen = np.sum(producto_sen)
        
        # señal demodulada:
        senal_demodulada[i*mpp: (i+1)*mpp] = producto_cos + producto_sen
        
        # Criterio de decisión por detección de energía
        # Obtención de los bits considerando el mapeo de la modulación:

        if i % 3 == 0:
            if ((-(1+h)/2)*Es_cos <= Ep_cos <=(-h/2)*Es_cos and
                (-(1+h)/2)*Es_sen <= Ep_sen <=(-h/2)*Es_sen):
                bits_Rx[i] = 0  # Para el bit 1
                bits_Rx[i+1] = 0  # Para el bit 2
                bits_Rx[i+2] = 0  # Para el bit 3

            if (Ep_cos <= (-(1+h)/2)*Es_cos and
                (-h/2)*Es_sen <= Ep_sen <= (h/2)* Es_sen):
                bits_Rx[i] = 0  # Para el bit 1
                bits_Rx[i+1] = 0  # Para el bit 2
                bits_Rx[i+2] = 1  # Para el bit 3

            if ((-h/2)*Es_cos <= Ep_cos <= (h/2)*Es_cos and
                Ep_sen >= ((1+h)/2)*Es_sen):
                bits_Rx[i] = 0  # Para el bit 1
                bits_Rx[i+1] = 1  # Para el bit 2
                bits_Rx[i+2] = 0  # Para el bit 3

            if ((-(1+h)/2)*Es_cos <= Ep_cos <=(-h/2)*Es_cos and
                (h/2)*Es_sen <= Ep_sen <=((1+h)/2)*Es_sen):
                bits_Rx[i] = 0  # Para el bit 1
                bits_Rx[i+1] = 1  # Para el bit 2
                bits_Rx[i+2] = 1  # Para el bit 3
            ...
            if ((-h/2)*Es_cos <= Ep_cos <= (h/2)*Es_cos and
                Ep_sen <= (-(1+h)/2)*Es_sen):
                bits_Rx[i] = 1  # Para el bit 1
                bits_Rx[i+1] = 0  # Para el bit 2
                bits_Rx[i+2] = 0  # Para el bit 3 

            if ((h/2)*Es_cos <= Ep_cos <= ((1+h)/2)*Es_cos and 
                (-(1+h)/2)*Es_sen <= Ep_sen <=(-h/2)*Es_sen):
                bits_Rx[i] = 1  # Para el bit 1
                bits_Rx[i+1] = 0  # Para el bit 2
                bits_Rx[i+2] = 1  # Para el bit 3 
...
#Se obtendrá la imagen recuperada y la imagen enviada.
import numpy as np
import matplotlib.pyplot as plt
import time
...
senal_Tx, Pm, porta_coseno, porta_seno = modulador_8_PSK(bits_Tx, fc, mpp)

# 4. Se transmite las señales moduladas, por un canal ruidoso
senal_Rx = canal_ruidoso(senal_Tx, Pm, SNR)

# 5. Se desmodula la señal recibida del canal
bits_Rx, senal_demodulada = demodulador_8_PSK(senal_Rx, porta_coseno, porta_seno, mpp)
...
# Formas de Onda de las imágenes
...
N = 8 # posibilidades de la onda debido a la modulación 8-PSK

# Definición de las variables de la amplitud y parámetro h:
h = np.sqrt(2) / 2

# valores de la amplitud A_1
A_1 = [1, h, 0, -h, -1, -h, 0, h]

# valores de la amplitud A_2
A_2 = [0, h, 1, h, 0, -h, -1, -h]

# Matriz para guardar las combinaciones:
combinaciones = np.zeros((N, len(t)))
...
# Se obtiene la media de las N posibilidades en cada instante de tiempo
media_N = [np.mean(combinaciones[:, i]) for i in range(len(t))]

# Se grafica el promedio de las N combinaciones:
valor_real = plt.plot(t, media_N, lw=7, color='pink', label='Valor promedio de las realizaciones')

# Se obtiene el valor teórico esperado:
Teorico = np.mean(senal_Tx)*t

valor_teorico = plt.plot(t, Teorico, '--', lw=4, color='m', label='Valor teórico')

# Gráfica para los valores teoricos y las muestras
plt.title('Realizaciones del proceso $X(t)$')
plt.xlabel('t(s)')
plt.ylabel('$x_i(t)$')
plt.legend()
plt.show()
...
# Respecto a la estacionaridad esta se da cuando todas sus propiedades estadísticas no varían a través del tiempo, por lo que sería un proceso aleatorio estacionario como en este caso, dado que la oscilación se mantiene constante independientemente del tiempo. Por su parte la ergodicidad establece la igualdad que existe entre el promedio estadístico y el promedio temporal de un proceso aleatorio, lo cual consiste en una forma de estacionaridad que puede simplificar el análisis de dicho proceso, sin embargo es una forma que limita la estacionaridad y puede ser difícil comprobar que esta constituye una suposición razonable para efectos de cualquier situación. 
# 
# Respecto a la gráfica, esta posee variaciones nulas; es decir sus datos son constantes por lo esto indica que se trata de un proceso ergódico, ya que se dice que para que un proceso sea ergódico en su promedio es necesario que el promedio del proceso sea constante, lo cual se cumple si el proceso es estacionario.
...
# Se importa la libreria para la transformada de Fourier
from scipy.fft import fft

# Transformada de Fourier
fft_senal = fft(senal_Tx)
...
# Espacio de las frecuencias del eje x
f = np.linspace(0.0, 1.0/(2.0*Tm), Ms//2)

# Gráfica de la distribución de potencia en todas las frecuencias
plt.plot(f, 2.0/Ms * np.power(np.abs(fft_senal [0:Ms//2]), 2))
plt.xlim(0, 20000)
